 ...
 */
public class CDrawChart 
{
    public String draw(Map<String, CPair> map) 
    {
        ...
        while(iterator.hasNext()) {
            pair = (CPair)iterator.next();
            count = pair.getCount();
            BarChartPlot document = Plots.newBarChartPlot(Data.newData(count), LIMEGREEN, String.valueOf(pair.getNumber()));
            list.add(document);
        ...
        AxisLabels freq = AxisLabelsFactory.newAxisLabels("頻率", 5);
        freq.setAxisStyle(axisStyle);
        AxisLabels dir = AxisLabelsFactory.newAxisLabels("語料庫", 5);
